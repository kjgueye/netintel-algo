// Ported verbatim from NetIntel src/routes/ai-image-assets.ts. Deviations:
// helpers from ./lib; lazy Anthropic singleton; stub body from env; price is
// the local AI_IMAGE_PRICE constant ($0.25, same as the main repo).
import { Router, type Request, type Response } from "express";
import Anthropic from "@anthropic-ai/sdk";
import { timeouts } from "./lib/config.js";
import { ValidationError } from "./lib/validators.js";
import { pickField } from "./lib/field-aliases.js";
import {
  getImageProvider,
  ImageProviderError,
  type ImageGenResult,
...
//   3. renders via a swappable image provider (OpenAI in v1);
//   4. returns the image plus rich, agent-usable metadata.
//
// Pricing is a FLAT per-call price ($0.25). The caps below bound the worst-case
// upstream cost (~$0.08 image + ~$0.001 Haiku) so the flat price clears ~3×
// margin on every call.

export const aiImageRouter = Router();

export const AI_IMAGE_PRICE = "$0.25";
...
// Parse the model's JSON reply defensively — tolerate code fences and prose.
...
aiImageRouter.get("/ai-image/generate", (_req: Request, res: Response) => {
  res.status(402).json(paymentRequiredStub(AI_IMAGE_PRICE));
});

aiImageRouter.head("/ai-image/generate", (_req: Request, res: Response) => {
  res.status(402).end();
});
...
aiImageRouter.post("/ai-image/generate", async (req: Request, res: Response) => {
  try {
    const body = req.body ?? {};
    ...
    res.json({
      ...
      price: priceBlock(AI_IMAGE_PRICE),
    });
  } catch (err) {
