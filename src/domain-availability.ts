// Ported verbatim from NetIntel src/routes/domain-availability.ts. Deviations:
// helpers from ./lib; the runDomainAvailability export (used only by NetIntel's
// domain-due-diligence aggregator, not ported) is dropped.
import { Router, type Request, type Response } from "express";
import { queryDns, RECORD_TYPES } from "./lib/dns-resolvers.js";
import { validateDomain, ValidationError } from "./lib/validators.js";
import { timeouts } from "./lib/config.js";

export const domainAvailabilityRouter = Router();

export const DOMAIN_AVAILABILITY_PRICE = "$0.050";
...
domainAvailabilityRouter.get("/domain-availability/check", async (req: Request, res: Response) => {
  try {
    ...
    const results: DomainResult[] = settled.map((s, i) => {
      ...
      return {
        // settled is mapped 1:1 from domainsToCheck; ! for noUncheckedIndexedAccess
