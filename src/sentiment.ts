// Ported verbatim from NetIntel src/routes/sentiment.ts. Deviations: helpers
// from ./lib; lazy Anthropic singleton; stub body from env; PRICE here is the
// $0.03 specified for this service (main repo lists $0.002).
...
export const SENTIMENT_PRICE = "$0.03";
...
sentimentRouter.get("/sentiment/analyze", (_req: Request, res: Response) => {
  res.status(402).json(paymentRequiredStub(SENTIMENT_PRICE));
});
...
// Parse the model's JSON reply defensively — tolerate code fences and surrounding prose.
