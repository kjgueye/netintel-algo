...
// getImageProvider() — no change to src/routes/ai-image-assets.ts.
//
// Why OpenAI for v1: the project already calls third-party APIs over plain
// fetch (no extra SDK to add), DALL·E 3 returns a hosted image URL plus a
// model-`revised_prompt`, and it follows brand/style prompts well. The hosted
// URL is convenient but EXPIRES (~60 min) — that's documented in the route's
// manifest so agents know to download promptly.
...
export interface ImageGenResult {
  /** Hosted image URL. May expire — see provider notes. */
  imageUrl: string;
  /** Provider's rewritten prompt, if it exposes one (DALL·E 3 does). */
