...
import { applyServiceMetadata } from "./service-metadata.js";
import { declareDiscoveryExtension } from "@x402-avm/extensions";
import { pricing } from "./config.js";
import { gatewayAccepts, GATEWAY_BODY_CLAUSE, GATEWAY_PRICING_SENTENCE, GATEWAY_PRICING_SENTENCE_ALIAS, GATEWAY_DEDICATED_SENTENCE, GATEWAY_MODEL_EXAMPLE, gatewaySettlementFailedBody } from "./services/gateway-pricing.js";
import { dnsRouter } from "./routes/dns.js";
import { sslRouter } from "./routes/ssl.js";
import { sslCertQuickRouter } from "./routes/ssl-cert-quick.js";
...
import { openaiGpt4oRouter } from "./routes/openai-gpt-4o.js";
import { openaiGpt41Router } from "./routes/openai-gpt-4-1.js";
import { openaiGpt41MiniRouter } from "./routes/openai-gpt-4-1-mini.js";
import { openaiGpt4oMiniRouter } from "./routes/openai-gpt-4o-mini.js";
import { openaiGpt41NanoRouter } from "./routes/openai-gpt-4-1-nano.js";
import { openaiGpt55Router } from "./routes/openai-gpt-5-5.js";
import { openaiGpt54Router } from "./routes/openai-gpt-5-4.js";
import { openaiGpt54MiniRouter } from "./routes/openai-gpt-5-4-mini.js";
import { openaiGpt54NanoRouter } from "./routes/openai-gpt-5-4-nano.js";
import { openaiGpt51Router } from "./routes/openai-gpt-5-1.js";
import { openaiGpt5NanoRouter } from "./routes/openai-gpt-5-nano.js";
import { v1OpenAiCompatRouter } from "./routes/v1-openai-compat.js";
import { v1EmbeddingsRouter } from "./routes/v1-embeddings.js";
import { embeddingsLocalRouter } from "./routes/embeddings-local.js";
import { semanticRankRouter } from "./routes/semantic-rank.js";
import { openaiGpt52Router } from "./routes/openai-gpt-5-2.js";
import { openaiGpt56SolRouter } from "./routes/openai-gpt-5-6-sol.js";
import { openaiGpt56TerraRouter } from "./routes/openai-gpt-5-6-terra.js";
import { openaiGpt56LunaRouter } from "./routes/openai-gpt-5-6-luna.js";
...
import { textChunkRouter } from "./routes/text-chunk.js";
import { textStatsRouter } from "./routes/text-stats.js";
import { translateLongRouter } from "./routes/translate-long.js";
import { translateShortRouter } from "./routes/translate-short.js";
import { translateStructuredRouter } from "./routes/translate-structured.js";
import { translateBatchRouter } from "./routes/translate-batch.js";
...
import { webFetchRouter } from "./routes/web-fetch.js";
import { exaSearchRouter } from "./routes/exa-search.js";
import { exaContentsRouter } from "./routes/exa-contents.js";
import { exaAnswerRouter } from "./routes/exa-answer.js";
...
import { cryptoMarketRouter } from "./routes/crypto-market.js";
import { gasPriceRouter } from "./routes/gas-price.js";
import { cryptoPriceRouter } from "./routes/crypto-price.js";
import { cryptoOhlcRouter } from "./routes/crypto-ohlc.js";
import { currencyBatchRouter } from "./routes/currency-batch.js";
import { currencyHistoryRouter } from "./routes/currency-history.js";
import { tokenInfoRouter } from "./routes/token-info.js";
import { walletBalanceRouter } from "./routes/wallet-balance.js";
import { walletIntelRouter } from "./routes/wallet-intel.js";
import { ibanValidateRouter } from "./routes/iban-validate.js";
import { marketSnapshotRouter } from "./routes/market-snapshot.js";
import { convertRouter } from "./routes/convert.js";
import { moneyParseRouter } from "./routes/money-parse.js";
import { jsonRepairRouter } from "./routes/json-repair.js";
import { schemaValidateRouter } from "./routes/schema-validate.js";
import { schemaMapRouter } from "./routes/schema-map.js";
...
import { weatherCurrentRouter } from "./routes/weather-current.js";
...
import { predictionMarketRouter } from "./routes/prediction-market.js";
...
import { domainVetRouter } from "./routes/domain-vet.js";


// github-intel accepts the repo via query (GET) OR JSON body (POST). Both methods
// share the same pricing/description/schema, but each declares its OWN discovery
// extension: @x402 2.18 validates the bazaar blob per route method, and a
// query-style declare (the default) is only legal on GET — sharing one object
// across GET+POST made the POST route fail validation (and silently skip Bazaar
// indexing). The POST variant declares body-style (method+bodyType) instead.
const githubIntelInputSchema = {
  type: "object",
  properties: {
    repo: {
      type: "string",
...
const githubIntelInput = { repo: "expressjs/express" };
const githubIntelBase = {
  accepts: paidAccepts(pricing.githubIntel),
  description:
...
// GET: repo arrives as ?repo= → query-style discovery declare (the default).
const githubIntelRouteConfig = {
  ...githubIntelBase,
  extensions: {
    ...declareDiscoveryExtension({
      inputSchema: githubIntelInputSchema,
      input: githubIntelInput,
      output: githubIntelOutput,
    }),
  },
};
// POST: repo arrives as JSON body → body-style declare (bodyType discriminates;
// the middleware stamps the method from the route key).
const githubIntelPostRouteConfig = {
  ...githubIntelBase,
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
// /web/fetch — raw URL fetch, GET (?url=) or POST ({"url"}) — same dual-method
// pattern as github-intel/iban-validate: shared price/description/schema, each
// method declaring its own discovery extension (query-style on GET, body-style
// on POST). Keep the blob terse: it is echoed into the paymentPayload.
const webFetchInputSchema = {
  ...
  properties: {
    url: {
      type: "string",
      description:
        "Absolute http(s) URL to fetch (data API, open-data portal, blob file). Aliases: uri, target, link, u",
    },
    max_bytes: {
      type: "number",
...
// Live output of this exact call (2026-08-28); `pairs` abbreviated to one entry.
const webFetchInput = { url: "https://api.dexscreener.com/latest/dex/tokens/0x833589fcd6edb6e08f4c7c32d4f71b54bda02913" };
const webFetchBase = {
  accepts: paidAccepts(pricing.webFetch),
  description:
...
const webFetchOutput = {
  schema: {
    ...
    properties: {
      ...
      status: { type: "number", description: "upstream HTTP status (2xx)" },
      content_type: { type: ["string", "null"], description: "upstream media type" },
      bytes: { type: "number", description: "bytes returned (≤ max_bytes)" },
      truncated: { type: "boolean" },
      json: { type: ["object", "array", "null"], description: "parsed body when it is JSON" },
      text: { type: ["string", "null"], description: "raw body when it is not JSON" },
      findings: { type: "array" },
    },
  },
  example: {
      ...
      pairs: [
        {
          ...
          baseToken: { address: "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913", name: "USD Coin", symbol: "USDC" },
          quoteToken: { address: "0xd9aAEc86B65D86f6A7B5B1b0c42FFA531710b6CA", name: "USD Base Coin", symbol: "USDbC" },
          priceUsd: "1.00013",
        },
      ],
    },
...
// GET: url arrives as ?url= → query-style declare. (The canonical-host resource
// pin drops the query string for discovery identity only; handling still reads ?url=.)
const webFetchRouteConfig = {
  ...webFetchBase,
  extensions: {
    ...declareDiscoveryExtension({
...
// POST: url arrives as JSON body → body-style declare.
const webFetchPostRouteConfig = {
  ...webFetchBase,
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
// /email/verify — boolean-first deliverability gate, GET (?email=) or POST
// ({"email"}) — same dual-method pattern as web/fetch above. Thin presentation
// of /email-intel/analyze's existing logic (shares runEmailIntel) under the
// name/price agents actually search for. See EXA-SEARCH-HANDOFF.md spec 24.
const emailVerifyInputSchema = {
  type: "object",
  properties: {
    email: {
      type: "string",
      description:
        "Email address to verify, 3-320 chars. Aliases: address, email_address, emailAddress, e, q, to",
    },
  },
  required: ["email"],
};
const emailVerifyInput = { email: "user@github.com" };
const emailVerifyBase = {
  accepts: paidAccepts(pricing.emailVerify),
  description:
...
const emailVerifyOutput = {
  schema: {
    ...
    properties: {
      ...
      result: { type: "string", description: "deliverable | risky | undeliverable" },
      reason: { type: "string" },
...
// GET: email arrives as ?email= → query-style declare.
const emailVerifyRouteConfig = {
  ...emailVerifyBase,
  extensions: {
    ...declareDiscoveryExtension({
      inputSchema: emailVerifyInputSchema,
      input: emailVerifyInput,
      output: emailVerifyOutput,
    }),
  },
};
// POST: email arrives as JSON body → body-style declare.
const emailVerifyPostRouteConfig = {
  ...emailVerifyBase,
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
// /exa/search + alias /web/search — Exa neural web search resale, GET (?query=)
// or POST ({"query"}) — same dual-method pattern as web/fetch above. ONE
// schema/input/output shared by both paths (only the description differs, per
// the Bazaar-brand-query play — see EXA-SEARCH-HANDOFF.md). Keep the blob
// terse: FOUR route keys share this schema, and it is echoed into the
// paymentPayload (memory x402-paymentpayload-size-limit).
const exaSearchInputSchema = {
  type: "object",
  properties: {
    query: { type: "string", description: "Search query, 1-1000 chars. Aliases: q, search, prompt, text" },
    num_results: {
      type: "number",
      description: "1-10 results (default 10); >10 clamped. Aliases: numResults, limit, n, count",
    },
    type: { type: "string", description: "auto (default) | neural | keyword | fast" },
    category: {
      type: "string",
      description: "company | research paper | news | pdf | github | tweet | personal site | linkedin profile | financial report",
    },
    include_domains: { type: "array", items: { type: "string" }, description: "Up to 20 hostnames to restrict to" },
    start_published_date: { type: "string", description: "ISO 8601 date/datetime lower bound" },
    end_published_date: { type: "string", description: "ISO 8601 date/datetime upper bound" },
    snippets: { type: "string", description: "none (default) | highlights | text — clamps num_results to 2" },
  },
  required: ["query"],
};
const exaSearchInput = { query: "x402 payment protocol", num_results: 5 };
const exaSearchOutput = {
  schema: {
    type: "object",
    properties: {
      query: { type: "string" },
...
const exaSearchRouteConfig = {
  accepts: paidAccepts(pricing.exaSearch),
  description:
    "Exa neural web search for AI agents — POST or GET a query, get ranked live results (title, url, published_date, author, score) with optional highlight snippets. Filter by category (news, research paper, github, company, pdf, and more), domain include/exclude, and publish date. For a direct answer use /exa/answer. No API key, pay per call on Base or Solana.",
  mimeType: "application/json",
  extensions: {
    ...declareDiscoveryExtension({
...
const exaSearchPostRouteConfig = {
  accepts: paidAccepts(pricing.exaSearch),
  description: exaSearchRouteConfig.description,
  mimeType: "application/json",
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
const webSearchPostRouteConfig = {
  accepts: paidAccepts(pricing.exaSearch),
  description: webSearchRouteConfig.description,
  mimeType: "application/json",
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
// /exa/contents — Exa page-contents batch resale, GET (?urls=) or POST
// ({"urls":[...]}) — same dual-method pattern as exa/search above. Closes the
// search -> contents loop /exa/search opens; positioned as /web/extract's
// "when direct fetch fails" complement (Cloudflare-walled/JS-rendered pages,
// batches up to 3 URLs). Keep the blob terse — it is echoed into the
// paymentPayload (memory x402-paymentpayload-size-limit).
...
const exaContentsInput = { urls: ["https://www.x402.org/"], max_characters: 2000 };
const exaContentsOutput = {
  schema: {
    type: "object",
    properties: {
      results: {
...
const exaContentsBase = {
  accepts: paidAccepts(pricing.exaContents),
  description:
...
const exaContentsRouteConfig = {
  ...exaContentsBase,
  extensions: {
    ...declareDiscoveryExtension({
      inputSchema: exaContentsInputSchema,
      input: exaContentsInput,
      output: exaContentsOutput,
    }),
  },
};
const exaContentsPostRouteConfig = {
  ...exaContentsBase,
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
// /exa/answer — Exa web-grounded answer resale, GET (?query=) or POST
// ({"query"}) — same dual-method pattern as exa/search and exa/contents
// above. Exa does retrieval AND generation; no OpenAI call on our side. Keep
// the blob terse — it is echoed into the paymentPayload (memory
// x402-paymentpayload-size-limit).
const exaAnswerInputSchema = {
  type: "object",
  properties: {
    query: {
      type: "string",
    ...
    include_citation_text: {
      type: "boolean",
      description:
        "Include each citation's source text (default false); truncated to 800 chars when >5 citations. Aliases: text, with_text",
    },
  },
  required: ["query"],
};
const exaAnswerInput = { query: "What is the x402 payment protocol?" };
const exaAnswerOutput = {
  schema: {
    type: "object",
    properties: {
      query: { type: "string" },
  ...
  example: {
    ...
    answer:
      "x402 is an open standard that uses the HTTP 402 status code to let clients pay for resources with stablecoins… [1][2]",
    citations: [
      {
        index: 1,
        title: "x402 — internet-native payments",
...
const exaAnswerBase = {
  accepts: paidAccepts(pricing.exaAnswer),
  description:
...
const exaAnswerPostRouteConfig = {
  ...exaAnswerBase,
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
// iban-validate accepts the IBAN via query (GET) OR JSON body (POST) — same
// dual-method pattern as github-intel above: shared pricing/description/schema,
// but each method declares its OWN discovery extension (query-style declare is
// only legal on GET; the POST variant declares body-style). POST is the
// documented-first shape for privacy-conscious callers: an IBAN is quasi-PII
// and GET query strings reach the paid-call logger's path+query metadata.
const ibanValidateInputSchema = {
  type: "object",
  properties: {
    iban: {
      type: "string",
      description:
        "The IBAN to validate — spaces, hyphens, case, and a leading IBAN: label are tolerated. Aliases: account, account_number, number",
    },
  },
  required: ["iban"],
};
// The canonical documentation IBAN (industry-standard example, not a real account).
const ibanValidateInput = { iban: "DE89370400440532013000" };
const ibanValidateBase = {
  accepts: paidAccepts(pricing.ibanValidate),
  // Keep under ~450 chars — oversized discovery blobs are echoed into the
  // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
  description:
...
const ibanValidateOutput = {
  schema: {
    type: "object",
    properties: {
      iban: { type: "string", description: "normalized electronic format" },
      valid: { type: "boolean" },
      country: { type: "string" },
      ...
      // Not every country format defines these — typed REALLY nullable or the
      // Bazaar indexer's strict validation kills indexing (memory
      // bazaar-strict-schema-validation).
      bank_code: { type: ["string", "null"] },
      branch_code: { type: ["string", "null"] },
      account_number: { type: ["string", "null"] },
      formatted: { type: "string", description: "print format, grouped by 4" },
...
// GET: iban arrives as ?iban= → query-style discovery declare (the default).
const ibanValidateRouteConfig = {
  ...ibanValidateBase,
  extensions: {
    ...declareDiscoveryExtension({
      inputSchema: ibanValidateInputSchema,
      input: ibanValidateInput,
      output: ibanValidateOutput,
    }),
  },
};
// POST: iban arrives as JSON body → body-style declare (bodyType discriminates;
// the middleware stamps the method from the route key).
const ibanValidatePostRouteConfig = {
  ...ibanValidateBase,
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
// text-stats accepts the text via query (GET) OR JSON body (POST) — same
// dual-method pattern as iban-validate above: shared pricing/description/
// schema, but each method declares its OWN discovery extension (query-style
// declare is only legal on GET; the POST variant declares body-style). POST is
// the documented shape for real payloads (200k-char cap); GET works for short
// strings. Pure deterministic utility — no upstream, no LLM (see the route).
const textStatsInputSchema = {
  type: "object",
  properties: {
    text: {
      type: "string",
...
const textStatsInput = {
  text: "NetIntel is a pay-per-call API for AI agents. It offers over one hundred endpoints.",
};
const textStatsBase = {
  accepts: paidAccepts(pricing.textStats),
  // Keep under ~450 chars — oversized discovery blobs are echoed into the
  // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
  description:
...
const textStatsOutput = {
  schema: {
    type: "object",
    properties: {
      characters: { type: "number", description: "Unicode code points, including whitespace" },
      characters_no_spaces: { type: "number", description: "Code points excluding all Unicode whitespace" },
      words: { type: "number", description: "Whitespace-delimited tokens" },
      unique_words: {
        type: "number",
      ...
      sentences: {
        type: "number",
        description:
          "Segments ended by . ! ? … followed by whitespace/end (runs collapse; a trailing segment without terminator counts); min 1",
      },
      paragraphs: { type: "number", description: "Blocks separated by one or more blank lines; min 1" },
      avg_word_length: { type: "number", description: "characters_no_spaces / words, 2 dp" },
      avg_sentence_length: { type: "number", description: "words / sentences, 2 dp" },
      // Null only when no token survives punctuation stripping (e.g. "..."):
      // typed REALLY nullable or the Bazaar indexer's strict validation kills
      // indexing (memory bazaar-strict-schema-validation).
      longest_word: {
        type: ["string", "null"],
        description: "Longest punctuation-stripped word, first seen on ties; null when nothing survives stripping",
      },
      reading_time_seconds: { type: "number", description: "ceil(words / 200 wpm × 60)" },
      speaking_time_seconds: { type: "number", description: "ceil(words / 130 wpm × 60)" },
      findings: { type: "array", description: "Reserved — currently always [] on success" },
    },
  },
  // Real response for textStatsInput (run through the handler, not hand-typed).
  example: {
    characters: 83,
...
// GET: text arrives as ?text= → query-style discovery declare (the default).
const textStatsRouteConfig = {
  ...textStatsBase,
  extensions: {
    ...declareDiscoveryExtension({
      inputSchema: textStatsInputSchema,
      input: textStatsInput,
      output: textStatsOutput,
    }),
  },
};
// POST: text arrives as JSON body → body-style declare (bodyType discriminates;
// the middleware stamps the method from the route key).
const textStatsPostRouteConfig = {
  ...textStatsBase,
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
// weather-current accepts a city OR lat/lon via query (GET) or JSON body (POST)
// — same dual-method pattern as iban-validate above: shared pricing/description/
// schema, but each method declares its OWN discovery extension (query-style
// declare is only legal on GET; the POST variant declares body-style).
const weatherCurrentInputSchema = {
  type: "object",
  properties: {
    city: {
      type: "string",
      description:
        'City or place name to geocode, e.g. "London" or "Austin, TX". Aliases: location, q, place, name. Ignored when coordinates are given',
    },
    latitude: {
      type: "number",
      description:
        "Latitude -90..90 — must be paired with longitude; coordinates skip geocoding and win over city. Alias: lat",
    },
    longitude: {
      type: "number",
...
const weatherCurrentInput = { city: "London" };
const weatherCurrentBase = {
  accepts: paidAccepts(pricing.weatherCurrent),
  // Keep under ~450 chars — oversized discovery blobs are echoed into the
  // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
  description:
  ...
  schema: {
    ...
    properties: {
      location: {
        type: "object",
        properties: {
          // name/country/region are null when coordinates are passed directly
          // (no geocode) — typed REALLY nullable or the Bazaar indexer's strict
          // validation kills indexing (memory bazaar-strict-schema-validation).
      ...
      current: {
        ...
        properties: {
          ...
          wind_direction_deg: { type: ["number", "null"] },
          precipitation: { type: ["number", "null"] },
          cloud_cover_pct: { type: ["number", "null"] },
          weather_code: { type: ["number", "null"] },
          condition: { type: ["string", "null"], description: "plain-language WMO condition" },
          is_day: { type: "boolean" },
        },
      },
      forecast: { type: "array", description: "next 3 days: date, temp_max, temp_min, precipitation, weather_code, condition" },
      units: {
        type: "object",
        properties: {
      ...
      source: { type: "string" },
      cache_age_seconds: { type: "number", description: "0 = fresh; forecasts are cached up to 10 min per location" },
      findings: { type: "array" },
    },
  },
  example: {
    ...
    forecast: [
      { date: "2026-08-31", temp_max: 18.4, temp_min: 11.2, precipitation: 1.3, weather_code: 61, condition: "Rain" },
      { date: "2026-09-01", temp_max: 19.1, temp_min: 12.0, precipitation: 0, weather_code: 2, condition: "Partly cloudy" },
      { date: "2026-09-02", temp_max: 17.8, temp_min: 11.6, precipitation: 0.4, weather_code: 80, condition: "Rain showers" },
    ],
...
// GET: params arrive as ?city= etc. → query-style discovery declare (the default).
const weatherCurrentRouteConfig = {
  ...weatherCurrentBase,
  extensions: {
    ...declareDiscoveryExtension({
      inputSchema: weatherCurrentInputSchema,
      input: weatherCurrentInput,
      output: weatherCurrentOutput,
    }),
  },
};
// POST: params arrive as a JSON body → body-style declare (bodyType discriminates;
// the middleware stamps the method from the route key).
const weatherCurrentPostRouteConfig = {
  ...weatherCurrentBase,
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
// weather-forecast: the multi-day + hourly planning companion to
// weather-current — same dual-method GET/POST pattern (query-style declare on
// GET, body-style on POST) with shared pricing/description/schema.
const weatherForecastInputSchema = {
  type: "object",
  properties: {
    city: {
      type: "string",
    ...
    days: {
      type: "number",
      description:
        "Daily forecast length 1-16 (default 7; out-of-range values are clamped with a finding). Alias: forecast_days",
    },
    hourly: {
      type: "boolean",
      description:
        "true to include the next 24 hours hour-by-hour (temperature, precipitation + probability, wind, condition). Default false. Alias: include_hourly; 1/true accepted",
    },
    units: {
      type: "string",
      description: "metric (default: °C, km/h, mm) or imperial (°F, mph, inch). Alias: unit",
...
const weatherForecastInput = { city: "London", days: 7 };
const weatherForecastBase = {
  accepts: paidAccepts(pricing.weatherForecast),
  // Keep under ~450 chars — oversized discovery blobs are echoed into the
  // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
  description:
...
const weatherForecastOutput = {
  schema: {
    ...
    properties: {
      ...
      daily: {
        type: "array",
        description:
          "one object per day: date, temp_max, temp_min, feels_like_max, feels_like_min, precipitation, precip_probability_pct (nullable far out), uv_index_max, sunrise, sunset, wind_speed_max, wind_gusts_max, wind_direction_deg, weather_code, condition",
      },
      hourly: {
        type: "array",
        description:
          "present ONLY when hourly=true — next 24 hours: time, temperature, precipitation, precip_probability_pct, wind_speed, weather_code, condition",
      },
      units: {
        type: "object",
        properties: {
      ...
      days: { type: "number", description: "daily forecast length actually used (after clamping to 1-16)" },
      source: { type: "string" },
      cache_age_seconds: { type: "number", description: "0 = fresh; forecasts are cached up to 10 min per location" },
      findings: { type: "array" },
    },
  ...
  example: {
    ...
    // Abbreviated to the first 2 of 7 rows to keep the discovery blob terse
    // (memory x402-paymentpayload-size-limit) — every row has this exact shape.
    daily: [
      {
        date: "2026-08-31",
        temp_max: 18.4,
...
// GET: params arrive as ?city= etc. → query-style discovery declare (the default).
const weatherForecastRouteConfig = {
  ...weatherForecastBase,
  extensions: {
    ...declareDiscoveryExtension({
...
// POST: params arrive as a JSON body → body-style declare (bodyType discriminates;
// the middleware stamps the method from the route key).
const weatherForecastPostRouteConfig = {
  ...weatherForecastBase,
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
// prediction-markets: live Polymarket odds — same dual-method GET/POST pattern
// as weather (query-style declare on GET, body-style on POST, shared base).
const predictionMarketsInputSchema = {
  type: "object",
  properties: {
    query: {
      type: "string",
      description:
        'Keyword search, e.g. "election" or "bitcoin". Aliases: q, search, keyword. Omit to list the top active markets by volume',
    },
    limit: {
      type: "number",
...
const predictionMarketsInput = { query: "bitcoin", limit: 2 };
const predictionMarketsBase = {
  accepts: paidAccepts(pricing.predictionMarkets),
  // Keep under ~450 chars — oversized discovery blobs are echoed into the
  // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
  description:
...
const predictionMarketsOutput = {
  schema: {
    type: "object",
    properties: {
      source: { type: "string" },
      query: { type: ["string", "null"], description: "echoed keyword; null when browsing top markets" },
      status: { type: "string", description: "effective filter: active, closed, or all" },
      count: { type: "number" },
      markets: {
        type: "array",
        ...
        items: {
          ...
          properties: {
            ...
            outcomes: {
              type: "array",
              description:
                "each {outcome, price}; price is the IMPLIED PROBABILITY 0..1 (0.55 = 55% chance), i.e. the current share price in USD",
            },
            volume_usd: { type: "number", description: "lifetime traded volume" },
            volume_24h_usd: { type: ["number", "null"] },
            liquidity_usd: { type: ["number", "null"] },
            end_date: { type: ["string", "null"], description: "ISO 8601 resolution/close date" },
            // Gamma no longer exposes a market category; derived from the event's
            // tags in search results, null when nothing maps — typed REALLY
            // nullable (memory bazaar-strict-schema-validation).
            category: { type: ["string", "null"] },
            active: { type: "boolean" },
            closed: { type: "boolean", description: "true once resolved/closed (active may still be true)" },
            url: { type: ["string", "null"] },
          },
        },
      },
      cache_age_seconds: { type: "number", description: "0 = fresh; results are cached up to 60s per query" },
      findings: { type: "array", description: "no_results, limit_clamped, invalid_status, outcomes_unparseable" },
    },
  },
  example: {
    source: "polymarket",
    ...
    markets: [
      {
        ...
        outcomes: [
          { outcome: "Yes", price: 0.0005 },
          { outcome: "No", price: 0.9995 },
        ],
...
// GET: params arrive as ?query= etc. → query-style discovery declare (the default).
const predictionMarketsRouteConfig = {
  ...predictionMarketsBase,
  extensions: {
    ...declareDiscoveryExtension({
      inputSchema: predictionMarketsInputSchema,
      input: predictionMarketsInput,
      output: predictionMarketsOutput,
    }),
  },
};
// POST: params arrive as a JSON body → body-style declare.
const predictionMarketsPostRouteConfig = {
  ...predictionMarketsBase,
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
// prediction-market: single-market drill-in for /prediction/markets — same
// dual-method GET/POST pattern (query-style declare on GET, body-style on POST).
const predictionMarketInputSchema = {
  type: "object",
  properties: {
    id: {
      type: "string",
    ...
    slug: {
      type: "string",
      description:
        'Market slug, e.g. "will-the-us-invade-iran-before-2027" (last segment of a polymarket.com URL; a full URL works too). Alias: market_slug',
    },
  },
};
const predictionMarketInput = { slug: "will-the-us-invade-iran-before-2027" };
const predictionMarketBase = {
  accepts: paidAccepts(pricing.predictionMarket),
  // Keep under ~450 chars — oversized discovery blobs are echoed into the
  // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
  description:
  ...
  schema: {
    ...
    properties: {
      ...
      outcomes: {
        type: "array",
        description:
          "each {outcome, price}; price = IMPLIED PROBABILITY 0..1 (0.55 = 55% chance)",
      },
      volume_usd: { type: "number", description: "lifetime traded volume" },
      volume_24h_usd: { type: ["number", "null"] },
      liquidity_usd: { type: ["number", "null"] },
      ...
      end_date: { type: ["string", "null"], description: "ISO 8601 resolution/close date" },
      // Polymarket leaves resolutionSource empty on most markets (the terms are
      // spelled out in description) — typed REALLY nullable, like category
      // (memory bazaar-strict-schema-validation).
      resolution_source: { type: ["string", "null"], description: "usually null — terms live in description" },
      category: { type: ["string", "null"], description: "from Polymarket tags" },
      active: { type: "boolean" },
      closed: { type: "boolean", description: "true once resolved (active may still be true)" },
      url: { type: ["string", "null"] },
      cache_age_seconds: { type: "number", description: "0 = fresh; cached <=60s per market" },
      findings: { type: "array", description: "outcomes_unparseable, id_treated_as_slug, slug_extracted_from_url" },
    },
  },
  // Real response captured live 2026-08-31 for the input example above.
  example: {
    source: "polymarket",
    id: "665374",
    ...
    outcomes: [
      { outcome: "Yes", price: 0.155 },
      { outcome: "No", price: 0.845 },
    ],
...
// GET: params arrive as ?slug= / ?id= → query-style discovery declare (the default).
const predictionMarketRouteConfig = {
  ...predictionMarketBase,
  extensions: {
    ...declareDiscoveryExtension({
      inputSchema: predictionMarketInputSchema,
      input: predictionMarketInput,
      output: predictionMarketOutput,
    }),
  },
};
// POST: params arrive as a JSON body → body-style declare.
const predictionMarketPostRouteConfig = {
  ...predictionMarketBase,
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
// Paid route entry for the /api/v1 alias of the OpenAI-compat gateway. A
// FUNCTION on purpose (see the note at its use in `routes`): call-expression
// values are skipped by the sync-ecosystem parser, keeping the alias out of
// README/MCP while runtime paywall + discovery still see it. Fresh object per
// call so the canonical-host resource pin stamps each path correctly.
const v1ChatGatewayAliasRoute = () => ({
  accepts: gatewayAccepts(),
  // @x402 2.18 sends an EMPTY body on a settlement failure unless a route
  // supplies this; without it a drained-wallet revert reads as "transient,
  // retry" (see src/services/gateway-pricing.ts).
  settlementFailedResponseBody: gatewaySettlementFailedBody,
  // Model list AND price sentence rendered from the dispatcher's own eligibility
  // filter (see GATEWAY_ADVERTISED_MODELS / GATEWAY_PRICING_SENTENCE) — never
  // hand-type model names or prices here.
  description: `OpenAI-compatible chat completions gateway (alias of /v1/chat/completions for api/v1-style base URLs) — standard request shape, model chosen in the body${GATEWAY_BODY_CLAUSE}. ${GATEWAY_PRICING_SENTENCE_ALIAS} Supports response_format json_object. Free model catalog: GET /v1/models.`,
  mimeType: "application/json",
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
      inputSchema: {
        type: "object",
        properties: {
          model: { type: "string", description: `Model id, e.g. ${GATEWAY_MODEL_EXAMPLE}. Free catalog: GET /v1/models` },
          messages: {
            type: "array",
          ...
          max_tokens: { type: "number", description: "Max output tokens (clamped to the model's cap)." },
        },
        required: ["model", "messages"],
      },
      input: { model: "gpt-4o-mini", messages: [{ role: "user", content: "Say hi in one word." }], max_tokens: 16 },
      output: {
        schema: {
          type: "object",
          properties: {
...
// Paid route entry for the /api/v1 alias of /v1/embeddings. A FUNCTION for the
// same reason as v1ChatGatewayAliasRoute above: call-expression values are
// skipped by the sync-ecosystem parser, keeping the alias out of README/MCP
// while runtime paywall + discovery still see it.
const v1EmbeddingsAliasRoute = () => ({
  accepts: paidAccepts(pricing.v1Embeddings),
  description:
    ...
    ...declareDiscoveryExtension({
      ...
      inputSchema: {
        ...
        properties: {
          ...
          dimensions: { type: "number", description: "Optional output vector size (truncation)." },
        },
        required: ["input"],
      },
      input: { model: "text-embedding-3-small", input: "the quick brown fox" },
      output: {
        schema: {
          type: "object",
          properties: {
            object: { type: "string" },
            data: { type: "array", description: "[{object:'embedding', index, embedding:[…]}]" },
            model: { type: "string" },
            usage: { type: "object" },
          },
        },
        example: {
          object: "list",
          data: [{ object: "embedding", index: 0, embedding: [0.0102, -0.0083, 0.0264] }],
...
// ssl-cert-quick: shared pricing/description/schema across the GET (query)
// and POST (JSON body) variants — same dual-method pattern as weather-current
// above (query-style discovery declare on GET, body-style on POST).
const sslCertQuickInputSchema = {
  type: "object",
  properties: {
    domain: {
      type: "string",
      description:
        "Domain to check (e.g. example.com) — a full URL is also accepted, the host is extracted. Aliases: host, hostname, url, site",
    },
    port: {
      type: "number",
...
const sslCertQuickInput = { domain: "example.com" };
const sslCertQuickBase = {
  accepts: paidAccepts(pricing.sslCertQuick),
  // Keep under ~450 chars — oversized discovery blobs are echoed into the
  // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
  description:
...
const sslCertQuickRouteConfig = {
  ...sslCertQuickBase,
  extensions: {
    ...declareDiscoveryExtension({
      inputSchema: sslCertQuickInputSchema,
      input: sslCertQuickInput,
      output: sslCertQuickOutput,
    }),
  },
};
const sslCertQuickPostRouteConfig = {
  ...sslCertQuickBase,
  extensions: {
    ...declareDiscoveryExtension({
      bodyType: "json",
...
const routes = {
  "GET /dns/lookup": {
    accepts: paidAccepts(pricing.dnsLookup),
    description:
      "DNS lookup for any domain — resolve A, AAAA, MX, TXT, NS, CNAME, SOA and PTR records in one call. Parses SPF/DMARC from TXT plus DKIM at the default._domainkey selector, and cross-checks A records across Google/Cloudflare/Quad9 resolvers for propagation consistency. Also answers nslookup / dig style queries.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
  ...
  "GET /ssl/analyze": {
    accepts: paidAccepts(pricing.sslAnalyze),
    description:
      "Performs a TLS handshake to inspect the certificate chain, probes supported TLS versions (1.0–1.3), detects weak keys and expiry issues, and returns an overall security grade (A/B/C/F). For just the cert facts fast, see the cheaper /ssl/cert (quick tier).",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
  ...
  "GET /ssl/cert": sslCertQuickRouteConfig,
  "POST /ssl/cert": sslCertQuickPostRouteConfig,
  "GET /redirect/trace": {
    accepts: paidAccepts(pricing.redirectTrace),
    description:
  ...
  "GET /security-headers/analyze": {
    accepts: paidAccepts(pricing.securityHeaders),
    description:
  ...
  "POST /email-auth": {
    accepts: paidAccepts(pricing.emailAuth),
    description:
  ...
  "GET /cloud-fingerprint/analyze": {
    accepts: paidAccepts(pricing.cloudFingerprint),
    description:
      "Fingerprints a domain's cloud infrastructure — DNS records, HTTP headers, a TLS handshake and PTR lookups — to detect CDN, WAF, hosting, DNS and email provider, plus cert issuer/wildcard/SAN count, with confidence scores and a security grade (A–F). Hosts that resolve but answer nothing on 80/443 return http_reachable:false, score null and grade insufficient_data — never a grade inferred from an unanswered probe.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
        ...
        output: {
            ...
            properties: {
              ...
              tls: {
                ...
                properties: {
                  ...
                  wildcard: { type: ["boolean", "null"] },
                  san_count: { type: ["number", "null"] },
                },
              },
              score: { type: ["number", "null"] },
              grade: { type: "string" },
          ...
          example: {
            ...
            tls: { issuer: "Google Trust Services", wildcard: true, san_count: 5 },
            score: 100,
            grade: "A",
            findings: [],
          },
  ...
  "POST /schema-parse/extract": {
    accepts: paidAccepts(pricing.schemaParse),
    description:
  ...
  "POST /messages": {
    accepts: paidAccepts(pricing.messages),
    description:
  ...
  "POST /openai/gpt-4o": {
    accepts: paidAccepts(pricing.openaiGpt4o),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      ...
      ...declareDiscoveryExtension({
        inputSchema: {
          ...
          properties: {
            messages: {
              type: "array",
              // Vision (this row only): content may be parts incl. <= 4 https image_url
              // parts per call; see src/services/openai-vision.ts. Kept terse — this
              // blob is echoed into every paymentPayload.
              description:
                "OpenAI chat messages: [{role, content}]. content is text, or parts: {type:'text',text} and (user role, max 4 per call) {type:'image_url',image_url:{url:'https://…',detail:'low'|'high'|'auto'}}. EVERY request — with or without images — must fit a 16000-token combined input budget and a 48000-char cap.",
              items: {
                type: "object",
                properties: {
                  role: { type: "string", description: "system, user, or assistant" },
                  // No `type` on purpose: content is a string OR an array of parts,
                  // and a union `type: ["string","array"]` is REFUSED by a strict
                  // ajv-2020 compile — the Bazaar indexer strict-validates this blob
                  // (memory bazaar-strict-schema-validation: an invalid blob settles
                  // fine but is silently never indexed). Typeless validates both forms.
                  content: { description: "Message text, or content parts: {type:'text',text} / {type:'image_url',image_url:{url,detail}}" },
                },
                required: ["role", "content"],
              },
            },
            ...
            top_p: { type: "number", description: "Nucleus sampling. Optional." },
          },
          required: ["messages"],
        },
        input: {
        ...
        output: {
          schema: {
            ...
            properties: {
              ...
              created: { type: "number", description: "Unix timestamp (seconds)" },
              model: { type: "string" },
              choices: { type: "array", description: "Assistant message + finish_reason." },
  ...
  "POST /openai/gpt-4-1": {
    accepts: paidAccepts(pricing.openaiGpt41),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-4.1 via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.09 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
          ...
          properties: {
            messages: {
              type: "array",
              description:
                "OpenAI chat messages: [{role:'system'|'user'|'assistant', content:string}]. Input capped ~48000 chars total.",
              items: {
                type: "object",
                properties: {
                  role: { type: "string", description: "system, user, or assistant" },
  ...
  "POST /openai/gpt-4-1-mini": {
    accepts: paidAccepts(pricing.openaiGpt41Mini),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
        ...
        inputSchema: {
          ...
          properties: {
            messages: {
              type: "array",
              description:
                "OpenAI chat messages: [{role:'system'|'user'|'assistant', content:string}]. Input capped ~12000 chars total.",
              items: {
                type: "object",
                properties: {
                  role: { type: "string", description: "system, user, or assistant" },
  ...
  // /api/v1 dialect alias of the gateway below — some OpenAI-SDK setups use
  // host/api/v1/... as the base URL (the miss-log showed probes on both
  // dialects). Built via a function call ON PURPOSE: the sync-ecosystem parser
  // ingests only object-literal route values, so the alias never becomes a
  // duplicate README row or MCP tool, while still being paywalled and
  // Bazaar-discoverable at runtime. MUST stay in this routes object — without
  // it the paymentMiddleware wouldn't paywall the alias path and SDKs pointed
  // at /api/v1 would get free completions.
  "POST /api/v1/chat/completions": v1ChatGatewayAliasRoute(),
  // /api/v1 dialect alias of /v1/embeddings below — same function-call trick as
  // the chat alias above (paywalled + discoverable, skipped by sync-ecosystem).
  "POST /api/v1/embeddings": v1EmbeddingsAliasRoute(),
  "POST /v1/embeddings": {
    accepts: paidAccepts(pricing.v1Embeddings),
    // Keep this discovery blob SMALL — see the note on the sibling openai routes.
    description:
        ...
        output: {
          schema: {
            ...
            properties: {
              ...
              data: { type: "array", description: "[{object:'embedding', index, embedding:[…]}]" },
              model: { type: "string" },
              usage: { type: "object", description: "prompt_tokens, total_tokens (input-only)" },
            },
          },
          example: {
            object: "list",
  ...
  "POST /embeddings": {
    accepts: paidAccepts(pricing.embeddingsLocal),
    // Keep this discovery blob SMALL — see the note on the sibling openai routes
    // (an oversized `extensions` blob gets echoed into the paymentPayload and
    // CDP /verify rejects it).
    description:
      ...
      ...declareDiscoveryExtension({
        ...
        inputSchema: {
          ...
          properties: {
            ...
            encoding_format: { type: "string", description: "float (default) or base64." },
          },
          required: ["input"],
        },
        input: { input: "the quick brown fox", input_type: "passage" },
        output: {
          schema: {
            type: "object",
            properties: {
              object: { type: "string", description: "Always 'list'" },
              data: { type: "array", description: "[{object:'embedding', index, embedding:[…384 floats]}]" },
              model: { type: "string", description: "Always 'netintel-embed-small'" },
              dimensions: { type: "number", description: "Always 384" },
              usage: { type: "object", description: "prompt_tokens, total_tokens (input-only, estimated)" },
            },
          },
          example: {
            object: "list",
  ...
  "POST /semantic/rank": {
    accepts: paidAccepts(pricing.semanticRank),
    // Keep this discovery blob SMALL — see the note on the sibling openai routes.
    description:
    ...
    extensions: {
      ...declareDiscoveryExtension({
        ...
        inputSchema: {
          type: "object",
          properties: {
            query: { type: "string", description: "The text to rank candidates against." },
            candidates: {
              type: "array",
            ...
            top_k: { type: "number", description: "Return only the best N results. Optional." },
            min_score: { type: "number", description: "Drop results scoring below this (-1 to 1). Optional." },
          },
        ...
        output: {
          schema: {
            ...
            properties: {
              ...
              method: { type: "string", description: "Always 'embedding_similarity'" },
              provider: { type: "string" },
              model: { type: "string" },
              dimensions: { type: "number" },
              normalized: { type: "boolean" },
              usage: { type: "object", description: "prompt_tokens (input-only)" },
            },
          },
          // Real response, round-tripped 2026-07-18 via scripts/audit-examples.mjs.
          example: {
            results: [
              { index: 0, score: 0.5361, text: "Use the Forgot Password link to recover your account." },
              { index: 1, score: 0.1399, text: "Refunds are accepted within 30 days." },
            ],
  ...
  "POST /v1/chat/completions": {
    ...
    settlementFailedResponseBody: gatewaySettlementFailedBody,
    // Keep this discovery blob SMALL — see the note on the sibling openai routes.
    // Model list rendered from the dispatcher's own eligibility filter (see
    // GATEWAY_MODEL_LIST) — never hand-type model names here: the old literal
    // advertised gpt-4o / gpt-4.1 / gpt-5.6-luna, all of which the gateway
    // rejects (18 logged 400s on 2026-09-07). Static readers resolve the
    // template via scripts/lib/description-expr.ts.
    description: `OpenAI-compatible chat completions gateway — standard /v1/chat/completions path and request shape, model chosen in the body${GATEWAY_BODY_CLAUSE}. ${GATEWAY_PRICING_SENTENCE} Supports response_format json_object. ${GATEWAY_DEDICATED_SENTENCE}GET /v1/models (free) lists every model with its endpoint and price.`,
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        bodyType: "json",
        inputSchema: {
          type: "object",
          properties: {
            model: { type: "string", description: `Model id, e.g. ${GATEWAY_MODEL_EXAMPLE}. Free catalog: GET /v1/models (every model, endpoint and price)` },
            messages: {
              type: "array",
  ...
  "POST /openai/gpt-4o-mini": {
    accepts: paidAccepts(pricing.openaiGpt4oMini),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-4o-mini via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.005 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
          ...
          properties: {
            messages: {
              type: "array",
              description:
                "OpenAI chat messages: [{role:'system'|'user'|'assistant', content:string}]. Input capped ~24000 chars total.",
              items: {
                type: "object",
                properties: {
                  role: { type: "string", description: "system, user, or assistant" },
  ...
  "POST /openai/gpt-4-1-nano": {
    accepts: paidAccepts(pricing.openaiGpt41Nano),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-4.1-nano via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.005 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
  ...
  "POST /openai/gpt-5-5": {
    accepts: paidAccepts(pricing.openaiGpt55),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-5.5 via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.65 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
          ...
          properties: {
            messages: {
              type: "array",
              description:
                "OpenAI chat messages: [{role:'system'|'user'|'assistant', content:string}]. Input capped ~96000 chars total.",
              items: {
                type: "object",
                properties: {
                  role: { type: "string", description: "system, user, or assistant" },
  ...
  "POST /openai/gpt-5-4": {
    accepts: paidAccepts(pricing.openaiGpt54),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-5.4 via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.25 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
  ...
  "POST /openai/gpt-5-4-mini": {
    accepts: paidAccepts(pricing.openaiGpt54Mini),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-5.4-mini via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.04 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
          ...
          properties: {
            messages: {
              type: "array",
              description:
                "OpenAI chat messages: [{role:'system'|'user'|'assistant', content:string}]. Input capped ~36000 chars total.",
              items: {
                type: "object",
                properties: {
                  role: { type: "string", description: "system, user, or assistant" },
  ...
  "POST /openai/gpt-5-4-nano": {
    accepts: paidAccepts(pricing.openaiGpt54Nano),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-5.4-nano via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.005 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
  ...
  "POST /openai/gpt-5-1": {
    accepts: paidAccepts(pricing.openaiGpt51),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-5.1 via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.15 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
  ...
  "POST /openai/gpt-5-nano": {
    accepts: paidAccepts(pricing.openaiGpt5Nano),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-5-nano via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.005 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
  ...
  "POST /openai/gpt-5-2": {
    accepts: paidAccepts(pricing.openaiGpt52),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-5.2 via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.20 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
  ...
  "POST /openai/gpt-5-6-sol": {
    accepts: paidAccepts(pricing.openaiGpt56Sol),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-5.6-sol via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.65 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
  ...
  "POST /openai/gpt-5-6-terra": {
    accepts: paidAccepts(pricing.openaiGpt56Terra),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-5.6-terra via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.25 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
  ...
  "POST /openai/gpt-5-6-luna": {
    accepts: paidAccepts(pricing.openaiGpt56Luna),
    // Keep this discovery blob SMALL and type-correct — the x402 client echoes
    // `extensions` into the paymentPayload sent to CDP /verify (oversized → every
    // payment rejected "paymentPayload invalid"), and the Bazaar indexer strictly
    // validates each example value against its declared type (mismatch → silently
    // never indexed). See memory x402-paymentpayload-size-limit + bazaar-strict-schema-validation.
    description:
      "Call OpenAI's gpt-5.6-luna via a single pay-per-call x402 endpoint — no OpenAI account or API key needed, pay $0.06 per request in USDC. Standard OpenAI chat.completions request/response shape, capped input and output.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
  ...
  "POST /ai-image/generate": {
    accepts: paidAccepts(pricing.aiImageAssets),
  ...
  "POST /classify": {
    accepts: paidAccepts(pricing.classify),
    description:
  ...
  "POST /content-moderate": {
    accepts: paidAccepts(pricing.contentModerate),
    description:
  ...
  "POST /entity-extract": {
    accepts: paidAccepts(pricing.entityExtract),
    description:
  ...
  "POST /extract/address": {
    accepts: paidAccepts(pricing.extractAddress),
    description:
  ...
  "POST /extract/contact": {
    accepts: paidAccepts(pricing.extractContact),
    description:
  ...
  "POST /extract/invoice": {
    accepts: paidAccepts(pricing.extractInvoice),
    description:
  ...
  "POST /extract/resume": {
    accepts: paidAccepts(pricing.extractResume),
    description:
  ...
  "POST /extract/table": {
    accepts: paidAccepts(pricing.extractTable),
    description:
  ...
  "POST /markdown/clean": {
    accepts: paidAccepts(pricing.markdownClean),
    description:
  ...
  "POST /normalize/json": {
    accepts: paidAccepts(pricing.normalizeJson),
    description:
  ...
  "POST /text-to-json": {
    accepts: paidAccepts(pricing.textToJson),
    description:
  ...
  "POST /sentiment/analyze": {
    accepts: paidAccepts(pricing.sentiment),
    description:
  ...
  "POST /text-summarize": {
    accepts: paidAccepts(pricing.textSummarize),
    description:
  ...
  "POST /text/chunk": {
    accepts: paidAccepts(pricing.textChunk),
    description:
      ...
      ...declareDiscoveryExtension({
        inputSchema: {
          ...
          properties: {
            ...
            chunk_size: {
              type: "number",
              description:
                "Window size in chars or words. Default 1000; clamped to 50–20000 chars or 5–20000 words. Aliases: size, chunkSize.",
            },
            overlap: {
              type: "number",
              description:
                "Units shared by consecutive chunks. Default 10% of chunk_size (max 100); clamped to 0..chunk_size-1. Aliases: chunk_overlap, chunkOverlap.",
            },
            unit: {
              type: "string",
        ...
        output: {
          schema: {
            type: "object",
            properties: {
              unit: { type: "string", description: "chars or words" },
              chunk_size: { type: "number", description: "Effective window size after clamping" },
              overlap: { type: "number", description: "Effective overlap after clamping" },
              count: { type: "number", description: "Number of chunks (max 2000)" },
              source_length: { type: "number", description: "Length of the input text in characters" },
              chunks: {
                type: "array",
                description:
                  "Sliding-window chunks in order. start/end are UTF-16 character offsets into the source (text.slice(start, end) reproduces a chars-mode chunk; words-mode chunks are re-joined with single spaces). Surrogate pairs are never split.",
                items: {
                  type: "object",
                  properties: {
                    index: { type: "number" },
                    ...
                    length: { type: "number", description: "Characters in this chunk's text" },
                  },
                },
              },
              findings: {
          ...
          example: {
            ...
            source_length: 115,
            chunks: [
              { index: 0, text: "NetIntel is a pay-per-call API for AI agents. It offers over", start: 0, end: 60, length: 60 },
              { index: 1, text: " It offers over one hundred endpoints. Agents pay per reques", start: 45, end: 105, length: 60 },
              { index: 2, text: " pay per request in USDC.", start: 90, end: 115, length: 25 },
            ],
            findings: [],
          },
        },
  ...
  "GET /text/stats": textStatsRouteConfig,
  // POST twin of /text/stats — same price/handler, body-style discovery
  // declare (see the config definitions above the routes object).
  "POST /text/stats": textStatsPostRouteConfig,
  "POST /translate/long": {
    accepts: paidAccepts(pricing.translateLong),
    description:
  ...
  "POST /translate/structured": {
    accepts: paidAccepts(pricing.translateStructured),
    description:
    ...
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
          ...
          properties: {
            content: {
              type: "object",
              description:
                "The structured content to translate: a JSON object/array, or a string of HTML, Markdown, or template text. Max 32KB / 500 strings.",
            },
            content_type: {
              type: "string",
              description:
                'One of "json", "html", "markdown", "template", "text". Auto-detected when omitted; a non-string content is always treated as json.',
            },
            target: {
              type: "string",
              description:
                'Target language — ISO 639-1 code ("fr") or English name ("French"). Aliases also accepted: target_lang, target_language, to, lang.',
            },
            source: {
              type: "string",
            ...
            glossary: {
              type: "object",
              description:
                'Enforced term mappings: {"dashboard":"tableau de bord"}. Compliance is reported per term; a missed term costs 10 points.',
            },
            protected_values: {
              type: "array",
              description:
                'Strings that must pass through byte-for-byte — brand names, IDs, SKUs: ["Acme","SKU-1234"]. Dropping one fails the call.',
            },
            formality: {
              type: "string",
              description: '"formal", "informal", or "neutral" (default).',
            },
            tone: { type: "string", description: "Optional tone guidance, e.g. \"friendly and concise\"." },
            locale: {
              type: "string",
        ...
        input: {
          content: { greeting: "Hello {{name}}", cta: "Click here to open the dashboard" },
          target: "fr",
        ...
        output: {
          schema: {
            ...
            properties: {
              ...
              detected_language: { type: "string", description: "ISO 639-1 code of the source language" },
              target_language: { type: "string", description: "ISO 639-1 code translated into" },
              content_type: { type: "string", description: "The content type used (given or auto-detected)" },
              placeholder_validation: {
                type: "object",
                description:
                  "Proof that every placeholder, tag, URL, email, code span, and protected value survived. A drop is a 502, never a 200.",
              },
              ...
              warnings: { type: "array", description: "Non-fatal notes, e.g. a missed glossary term" },
              score: { type: "number", description: "0-100; 100 on a clean, structure-valid translation" },
              grade: { type: "string", description: "Letter grade A-F derived from score" },
              findings: { type: "array", description: "Scored deductions, e.g. glossary_missed" },
            },
          },
          example: {
            translated: {
            ...
            structure_validation: { passed: true, type: "json" },
            glossary_compliance: { applied: ["dashboard→tableau de bord"], missed: [] },
            warnings: [],
            score: 100,
            grade: "A",
            findings: [],
  ...
  "POST /translate/batch": {
    accepts: paidAccepts(pricing.translateBatch),
    description:
      ...
      ...declareDiscoveryExtension({
        inputSchema: {
          ...
          properties: {
            items: {
              type: "array",
              description:
                'The strings to translate: [{"id":"greeting","text":"Hello {{name}}","context":"button label"}]. ids and order are preserved in the response. 1-200 items, 64KB total.',
            },
            target: {
              type: "string",
              description:
            ...
            tone: { type: "string", description: 'Optional tone guidance, e.g. "friendly and concise".' },
            formality: { type: "string", description: '"formal", "informal", or "neutral" (default).' },
          },
        ...
        input: {
          items: [
            ...
            { id: "cta", text: "Open the dashboard" },
          ],
          target: "fr",
          glossary: { dashboard: "tableau de bord" },
        },
        ...
        output: {
          schema: {
            ...
            properties: {
              target_language: { type: "string", description: "ISO 639-1 code translated into" },
              item_count: { type: "number", description: "Items submitted" },
              succeeded: { type: "number", description: "Items translated and structurally verified" },
              failed: { type: "number", description: "Items that failed — the batch still returns 200" },
              items: {
                type: "array",
                description:
                  'One entry per input item, in input order, each carrying your id: {id, status ("ok"|"failed"), translated, detected_language, confidence, warnings, reason}. A failed item has translated=null and a reason (structure_broken, unparseable, truncated, llm_unavailable, timeout).',
              },
              billed_items: { type: "number", description: "Item count used for pricing (base covers 25)" },
              pricing: {
                type: "object",
                description: "Base price, included items, and per-item overage beyond the included count",
              },
              usage: { type: "object", description: "Model, model_calls, input_tokens, output_tokens" },
              warnings: { type: "array", description: "Batch-level notes, e.g. that some items failed" },
              score: { type: "number", description: "0-100; 100 on a successful run, even with some items failed" },
              grade: { type: "string", description: "Letter grade A-F derived from score" },
              findings: { type: "array", description: "Scored deductions, e.g. all_items_failed" },
            },
          },
          example: {
            target_language: "fr",
            ...
            usage: { model: "claude-haiku-4-5-20251001", model_calls: 2, input_tokens: 480, output_tokens: 64 },
            warnings: [],
            score: 100,
            grade: "A",
            findings: [],
  ...
  "POST /translate/short": {
    accepts: paidAccepts(pricing.translateShort),
    description:
  ...
  "GET /asn-lookup/analyze": {
    accepts: paidAccepts(pricing.asnLookup),
    description:
  ...
  "GET /whois-rdap/lookup": {
    accepts: paidAccepts(pricing.whoisRdap),
    description:
      "Domain registration lookup, WHOIS via RDAP — registrar, registration and expiry dates, last-updated date, nameservers, and status flags from the authoritative registry, plus an actionability score so agents can assess domain trustworthiness, ownership, or availability risk.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
  ...
  "GET /cert-transparency/lookup": {
    accepts: paidAccepts(pricing.certTransparency),
    description:
  ...
  "GET /subnet/calc": {
    accepts: paidAccepts(pricing.subnetCalc),
    description:
  ...
  "GET /dns-propagation/check": {
    accepts: paidAccepts(pricing.dnsPropagation),
    description:
  ...
  "GET /dnssec/validate": {
    accepts: paidAccepts(pricing.dnssec),
    description:
        ...
        output: {
          schema: {
            ...
            properties: {
              ...
              resolver_validated: { type: "boolean", description: "Validating resolver's AD bit — chain verified upstream" },
          ...
          // Real response, re-captured 2026-07-19 after the DoH transport fix
          // (dns2/UDP couldn't fetch DS/DNSKEY — signed domains graded F).
          example: {
            domain: "example.com",
            dnssec_enabled: true,
            ...
            components: {
              ...
              dnskey_record: { present: true, count: 4, key_types: ["ZSK", "KSK", "ZSK", "ZSK"] },
              rrsig_record: { present: true, signatures_found: 1 },
              nsec_or_nsec3: { present: true, type: "NSEC" },
            },
            tld_ds_present: true,
            score: 100,
            grade: "A",
  ...
  "GET /ip-blacklist/check": {
    accepts: paidAccepts(pricing.ipBlacklist),
    description:
  ...
  "GET /ip-reputation/analyze": {
    accepts: paidAccepts(pricing.ipReputation),
    description:
      ...
      ...declareDiscoveryExtension({
        ...
        output: {
          schema: {
            ...
            properties: {
              ip: { type: "string" },
              risk_score: { type: "number", description: "0-100, higher = more malicious" },
  ...
  "GET /cron-parser/explain": {
    accepts: paidAccepts(pricing.cronParser),
    description:
  ...
  "GET /currency-exchange/convert": {
    // First dual-chain route: advertises Base AND (when SOLANA_PAY_TO is set)
    // USDC-on-Solana, settled by the same CDP facilitator. The client picks
    // whichever network its wallet speaks. Pilot for the all-routes rollout —
    // see memory: solana-x402-expansion.
    accepts: paidAccepts(pricing.currencyExchange),
  ...
  "GET /crypto/market": {
    accepts: paidAccepts(pricing.cryptoMarket),
    // Keep under ~450 chars — oversized discovery blobs are echoed into the
    // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
    description:
        ...
        output: {
          schema: {
            ...
            properties: {
              ...
              vs: { type: "string" },
              price: { type: "number" },
              // Degradable fields are REALLY nullable — the Bazaar indexer
              // strict-validates example vs schema, and a null-able field typed
              // non-null silently kills indexing (memory bazaar-strict-schema-validation).
              change_24h: { type: ["number", "null"] },
              change_24h_pct: { type: ["number", "null"] },
              high_24h: { type: ["number", "null"] },
              low_24h: { type: ["number", "null"] },
              open_24h: { type: ["number", "null"] },
              volume_24h: { type: ["number", "null"] },
              market_cap: { type: ["number", "null"] },
              market_cap_rank: { type: ["number", "null"] },
              supply: {
                type: "object",
                properties: {
                  circulating: { type: ["number", "null"] },
              ...
              cache_age_seconds: { type: "number" },
              source: { type: "string", description: "coinbase+coingecko | coinbase | coingecko" },
              findings: { type: "array" },
            },
          },
          example: {
            ...
            market_cap_rank: 1,
            supply: { circulating: 20058731, total: 20058734, max: 21000000 },
  ...
  "GET /gas/price": {
    accepts: paidAccepts(pricing.gasPrice),
    // Keep under ~450 chars — oversized discovery blobs are echoed into the
    // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
    description:
        ...
        inputSchema: {
          ...
          properties: {
            chains: {
              type: "string",
              description:
                "Comma list from base, ethereum, arbitrum, optimism, polygon; omit for all 5. Aliases: chain, network, networks; eth/arb/op/matic/pol accepted",
            },
          },
        },
        input: { chains: "base,ethereum" },
        output: {
          schema: {
            type: "object",
            properties: {
              chains: {
                type: "object",
                // A chain whose every RPC failed is a null entry (finding explains) —
                // typed nullable or the Bazaar indexer's strict validation kills
                // indexing (memory bazaar-strict-schema-validation).
                additionalProperties: {
                  ...
                  properties: {
                    ...
                    priority_fee_gwei: { type: ["number", "null"] },
                    usd_estimates: {
                      type: "object",
                      properties: {
                        transfer: { type: ["number", "null"] },
                        erc20_transfer: { type: ["number", "null"] },
                        swap: { type: ["number", "null"] },
                      },
                    },
                    native_token: { type: "string" },
                    native_usd: { type: ["number", "null"] },
                    source: { type: "string" },
                    cache_age_seconds: { type: "number" },
                  },
                },
          ...
          // Real captured response (audit-examples round-trip, 2026-08-01).
          example: {
            chains: {
              base: {
                ...
                usd_estimates: { transfer: 0.000230535, erc20_transfer: 0.000713562, swap: 0.00219557 },
                native_token: "ETH",
                native_usd: 1829.645,
              ...
              ethereum: {
                ...
                usd_estimates: { transfer: 0.0046079, erc20_transfer: 0.0142625, swap: 0.0438847 },
                native_token: "ETH",
                native_usd: 1829.645,
  ...
  "GET /crypto/price": {
    accepts: paidAccepts(pricing.cryptoPrice),
    // Keep under ~450 chars — oversized discovery blobs are echoed into the
    // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
    description:
    ...
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
          ...
          properties: {
            symbols: {
              type: "string",
              description:
                'Comma list of 1–25 tickers or full names ("BTC,ETH" or "bitcoin"); a leading $ is fine. Aliases: symbol, coins, coin, tickers, ticker, assets',
            },
            vs: {
              type: "string",
        ...
        // Keep the example small — 2 symbols, not 25 (paymentPayload size limit).
        ...
        output: {
              ...
              prices: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    name: { type: "string" },
                    price: { type: "number" },
                    // Only CoinGecko-answered rows carry a 24h change — typed
                    // REALLY nullable or the Bazaar indexer's strict validation
                    // kills indexing (memory bazaar-strict-schema-validation).
                    change_24h_pct: { type: ["number", "null"] },
                    source: { type: "string", description: "coinbase | coingecko | kraken" },
                    cache_age_seconds: { type: "number" },
                  },
                },
              },
          ...
          example: {
            vs: "USD",
            prices: [
              { symbol: "BTC", name: "Bitcoin", price: 62441.615, change_24h_pct: null, source: "coinbase", cache_age_seconds: 0 },
              { symbol: "ETH", name: "Ethereum", price: 1829.645, change_24h_pct: null, source: "coinbase", cache_age_seconds: 0 },
            ],
            unresolved: [],
            stale: false,
            findings: [],
  ...
  "GET /crypto/ohlc": {
    accepts: paidAccepts(pricing.cryptoOhlc),
    // Keep under ~450 chars — oversized discovery blobs are echoed into the
    // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
    description:
    ...
    extensions: {
      ...declareDiscoveryExtension({
        ...
        // Keep the example small — days=7, not 300 (paymentPayload size limit).
        input: { symbol: "BTC", interval: "1d", days: 7 },
        output: {
          schema: {
            type: "object",
            properties: {
              ...
              stats: {
                ...
                properties: {
                  ...
                  period_high: { type: "number" },
                  period_low: { type: "number" },
                  avg_volume: { type: "number" },
                },
              },
              // Present only in date mode (the requested day's close) — typed
              // nullable or the Bazaar indexer's strict validation kills
              // indexing (memory bazaar-strict-schema-validation).
              price: { type: ["number", "null"] },
              candle_count: { type: "number" },
              source: { type: "string", description: "coinbase | kraken" },
              cache_age_seconds: { type: "number" },
              stale: { type: "boolean" },
              findings: { type: "array" },
            },
          ...
          example: {
            ...
            candles: [
              { time: "2026-07-26T00:00:00.000Z", open: 64295.61, high: 65511.36, low: 64229.6, close: 65341.07, volume: 2390.56790196 },
              { time: "2026-07-27T00:00:00.000Z", open: 65341.07, high: 65705.08, low: 63545, close: 63694.43, volume: 5069.45269918 },
              { time: "2026-07-28T00:00:00.000Z", open: 63693.44, high: 64041.02, low: 62645.39, close: 63847.12, volume: 6232.18054403 },
              { time: "2026-07-29T00:00:00.000Z", open: 63847.12, high: 64665.3, low: 63194.45, close: 63896.14, volume: 8092.70552294 },
              { time: "2026-07-30T00:00:00.000Z", open: 63896.14, high: 65118.08, low: 63502.91, close: 64721.9, volume: 8025.87127027 },
              { time: "2026-07-31T00:00:00.000Z", open: 64721.9, high: 65360, low: 62358.96, close: 62825.9, volume: 8260.37090197 },
              { time: "2026-08-01T00:00:00.000Z", open: 62826.69, high: 63093, low: 62209.81, close: 62445.14, volume: 2472.01120363 },
            ],
            stats: { open: 64295.61, close: 62445.14, change_pct: -2.88, period_high: 65705.08, period_low: 62209.81, avg_volume: 5791.88 },
            candle_count: 7,
            source: "coinbase",
            cache_age_seconds: 0,
            stale: false,
  ...
  "GET /currency-exchange/batch": {
    accepts: paidAccepts(pricing.currencyBatch),
    // Keep under ~450 chars — oversized discovery blobs are echoed into the
    // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
    description:
    ...
    extensions: {
      ...declareDiscoveryExtension({
        ...
        // Keep the example small — 3 targets, not 30 (paymentPayload size limit).
        input: { from: "USD", to: "EUR,SAR,BTC", amount: 100 },
        output: {
          schema: {
            type: "object",
            properties: {
              ...
              rates: {
                ...
                items: {
                  ...
                  properties: {
                    to: { type: "string" },
                    // Failed targets stay in the list with nulls — typed REALLY
                    // nullable or the Bazaar indexer's strict validation kills
                    // indexing (memory bazaar-strict-schema-validation).
          ...
          example: {
            ...
            amount: 100,
            rates: [
              { to: "EUR", rate: 0.8707, converted_amount: 87.07, rate_date: "2026-07-31", source: "frankfurter" },
              { to: "SAR", rate: 3.75, converted_amount: 375, rate_date: "2026-08-01", source: "currency-api" },
              { to: "BTC", rate: 0.000016014961, converted_amount: 0.0016014961, rate_date: "2026-08-01", source: "coinbase" },
            ],
            unavailable: [],
            stale: false,
            findings: [],
  ...
  "GET /currency-exchange/history": {
    accepts: paidAccepts(pricing.currencyHistory),
    // Keep under ~450 chars — oversized discovery blobs are echoed into the
    // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
    description:
    ...
    extensions: {
      ...declareDiscoveryExtension({
        ...
        // Keep the example small — days=7, not 30 (paymentPayload size limit).
        input: { from: "USD", to: "EUR", days: 7 },
        output: {
          schema: {
            type: "object",
            properties: {
              ...
              series: {
                ...
                items: {
                  ...
                  properties: {
                    date: { type: "string" },
                    rate: { type: "number", description: "units of `to` per 1 `from` on that date" },
                  },
                },
              },
              stats: {
                ...
                properties: {
                  ...
                  max: { type: "number" },
                  average: { type: "number" },
                  change_pct: { type: "number", description: "first→last point, %" },
                  volatility_pct: { type: "number", description: "stdev of daily % changes" },
                },
              },
          ...
          // Real captured response (audit-examples round-trip, 2026-08-01).
          // Note: frankfurter maps a weekend range-start to the prior business
          // day, so the first series point can pre-date `start` — real behavior.
          example: {
            from: "USD",
            to: "EUR",
            start: "2026-07-26",
            ...
            series: [
              { date: "2026-07-24", rate: 0.87897 },
              { date: "2026-07-27", rate: 0.87804 },
              { date: "2026-07-28", rate: 0.87974 },
              { date: "2026-07-29", rate: 0.87873 },
              { date: "2026-07-30", rate: 0.87138 },
              { date: "2026-07-31", rate: 0.8707 },
            ],
            stats: { min: 0.8707, max: 0.87974, average: 0.87626, change_pct: -0.94, volatility_pct: 0.34 },
            point_count: 6,
            source: "frankfurter",
            cache_age_seconds: 0,
            stale: false,
  ...
  "GET /token/info": {
    accepts: paidAccepts(pricing.tokenInfo),
    // Keep under ~450 chars — oversized discovery blobs are echoed into the
    // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
    description:
        ...
        inputSchema: {
          ...
          properties: {
            address: {
              type: "string",
              description:
                "ERC-20 contract address: 0x + 40 hex chars, checksum case ignored. Aliases: contract, token, contract_address",
            },
            chain: {
              type: "string",
        ...
        output: {
          schema: {
            ...
            properties: {
              address: { type: "string" },
              chain: { type: "string" },
              // Weird tokens can revert name()/symbol()/decimals() — every
              // degradable field typed REALLY nullable or the Bazaar indexer's
              // strict validation kills indexing (memory bazaar-strict-schema-validation).
              name: { type: ["string", "null"] },
              ...
              source: { type: "string", description: 'e.g. "rpc+dexscreener+blockscout"' },
              cache_age_seconds: { type: "number" },
              stale: { type: "boolean" },
              findings: { type: "array" },
            },
  ...
  "GET /wallet/balance": {
    accepts: paidAccepts(pricing.walletBalance),
    // Keep under ~450 chars — oversized discovery blobs are echoed into the
    // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
    description:
    ...
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
          ...
          properties: {
            address: {
              type: "string",
              description:
                "Wallet address: 0x + 40 hex chars on base/ethereum, base58 on solana. Aliases: wallet, account, addr",
            },
            chain: {
              type: "string",
        ...
        input: { address: "0x4F3c04C3F17264B83ae884df7985142B7DC8047c", chain: "base" },
        output: {
          schema: {
            type: "object",
            properties: {
              ...
              native: {
                ...
                properties: {
                  symbol: { type: "string" },
                  raw: { type: "string", description: "raw units (wei/lamports) as a decimal string" },
                  formatted: { type: "number" },
                  // usd degrades to null when the spot price is down — typed
                  // REALLY nullable or the Bazaar indexer's strict validation
                  // kills indexing (memory bazaar-strict-schema-validation).
              ...
              source: { type: "string", description: "the RPC that answered" },
              cache_age_seconds: { type: "number" },
              stale: { type: "boolean" },
              findings: { type: "array" },
            },
          ...
          example: {
            ...
            chain: "base",
            native: { symbol: "ETH", raw: "0", formatted: 0, usd: 0 },
            usdc: { raw: "3519150", formatted: 3.51915, usd: 3.52 },
            tokens: [],
            source: "https://mainnet.base.org",
            cache_age_seconds: 0,
            stale: false,
  ...
  "GET /wallet/intel": {
    accepts: paidAccepts(pricing.walletIntel),
    // Keep under ~450 chars — oversized discovery blobs are echoed into the
    // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
    description:
        ...
        output: {
          schema: {
            ...
            properties: {
              address: { type: "string" },
              chain: { type: "string" },
              // Every derivable field degrades to null when a provider leg is
              // down — typed REALLY nullable or the Bazaar indexer's strict
              // validation kills indexing (memory bazaar-strict-schema-validation).
              is_contract: { type: ["boolean", "null"] },
              wallet_age_days: { type: ["number", "null"] },
              first_seen: { type: ["string", "null"] },
              first_funder: {
                type: ["object", "null"],
                properties: { address: { type: "string" }, date: { type: "string" } },
              },
              usdc_in_total: { type: ["number", "null"] },
              usdc_out_total: { type: ["number", "null"] },
              net_flow: { type: ["number", "null"] },
              counterparties: { type: ["number", "null"] },
              top_counterparty: {
                type: ["object", "null"],
                properties: { address: { type: "string" }, volume: { type: "number" } },
              },
              activity_30d: {
                type: ["object", "null"],
                properties: { transfers: { type: "number" }, volume: { type: "number" } },
              },
              activity_pattern: { type: ["string", "null"], description: "dormant | steady | burst" },
              native_balance: { type: ["number", "null"] },
              transfers_analyzed: { type: ["number", "null"] },
              truncated: { type: "boolean" },
              score: { type: ["number", "null"], description: "0-100; null when history is unavailable" },
              grade: { type: ["string", "null"], description: "A-F" },
          ...
          // Real captured response (audit-examples round-trip, 2026-08-01) —
          // the buyer wallet's genuine on-chain profile, no invented addresses.
          example: {
            address: "0x4f3c04c3f17264b83ae884df7985142b7dc8047c",
            chain: "base",
            is_contract: false,
            wallet_age_days: 157,
            first_seen: "2026-02-25T13:48:31.000Z",
            first_funder: { address: "0xdadc335482ad545296fd7b28518a251ffcbeb9df", date: "2026-02-25T13:48:31.000Z" },
            usdc_in_total: 31.00175,
            usdc_out_total: 25.691,
            net_flow: 5.31075,
            counterparties: 6,
            top_counterparty: { address: "0xdadc335482ad545296fd7b28518a251ffcbeb9df", volume: 31.691 },
            activity_30d: { transfers: 213, volume: 11.685 },
            activity_pattern: "steady",
            native_balance: 0,
            transfers_analyzed: 727,
            truncated: false,
            score: 100,
            grade: "A",
            flags: [],
  ...
  "GET /iban/validate": ibanValidateRouteConfig,
  // POST twin of /iban/validate — same price/handler, body-style discovery
  // declare (see the config definitions above the routes object).
  "POST /iban/validate": ibanValidatePostRouteConfig,
  "GET /weather/current": weatherCurrentRouteConfig,
  // POST twin of /weather/current — same price/handler, body-style discovery
  // declare (see the config definitions above the routes object).
  "POST /weather/current": weatherCurrentPostRouteConfig,
  "GET /weather/forecast": weatherForecastRouteConfig,
  // POST twin of /weather/forecast — same price/handler, body-style discovery
  // declare (see the config definitions above the routes object).
  "POST /weather/forecast": weatherForecastPostRouteConfig,
  "GET /prediction/markets": predictionMarketsRouteConfig,
  // POST twin of /prediction/markets — same price/handler, body-style discovery
  // declare (see the config definitions above the routes object).
  "POST /prediction/markets": predictionMarketsPostRouteConfig,
  "GET /prediction/market": predictionMarketRouteConfig,
  // POST twin of /prediction/market — same price/handler, body-style discovery
  // declare (see the config definitions above the routes object).
  "POST /prediction/market": predictionMarketPostRouteConfig,
  "GET /market/snapshot": {
    accepts: paidAccepts(pricing.marketSnapshot),
    // Keep under ~450 chars — oversized discovery blobs are echoed into the
    // paymentPayload and CDP rejects settlement. See memory x402-paymentpayload-size-limit.
    description:
        ...
        inputSchema: {
          ...
          properties: {
            symbols: {
              type: "string",
              description:
                'Optional override of the crypto list, up to 6 tickers or full names (default "BTC,ETH,SOL"). Aliases: coins, tickers',
            },
            vs: {
              type: "string",
        ...
        output: {
          schema: {
            ...
            properties: {
              ...
              vs: { type: "string" },
              // Each section is null when its upstreams all failed (finding
              // explains) — typed nullable or the Bazaar indexer's strict
              // validation kills indexing (memory bazaar-strict-schema-validation).
              crypto: {
                type: ["array", "null"],
                items: {
                  ...
                  properties: {
                    ...
                    high_24h: { type: ["number", "null"] },
                    low_24h: { type: ["number", "null"] },
                    source: { type: "string", description: "coinbase | kraken | coingecko" },
                  },
                },
              },
              fiat: {
          ...
          example: {
            ...
            vs: "USD",
            crypto: [
              { symbol: "BTC", price: 62445.13, change_24h_pct: -0.9224, high_24h: 63093, low_24h: 62209.81, source: "coinbase" },
              { symbol: "ETH", price: 1830.6, change_24h_pct: -2.094, high_24h: 1873.8, low_24h: 1820.14, source: "coinbase" },
              { symbol: "SOL", price: 70.84, change_24h_pct: -2.972, high_24h: 73.19, low_24h: 70.51, source: "coinbase" },
            ],
            fiat: [
              { pair: "USD/EUR", rate: 0.8707, rate_date: "2026-07-31" },
              { pair: "USD/GBP", rate: 0.74508, rate_date: "2026-07-31" },
              { pair: "USD/JPY", rate: 160.24, rate_date: "2026-07-31" },
              { pair: "EUR/GBP", rate: 0.855725, rate_date: "2026-07-31" },
            ],
            gas: [
              { chain: "base", gas_price_gwei: 0.006, transfer_usd: 0.000230535 },
              { chain: "ethereum", gas_price_gwei: 0.119926866, transfer_usd: 0.0046079 },
            ],
  ...
  "GET /convert": {
    accepts: paidAccepts(pricing.convert),
    description:
  ...
  "POST /money/parse": {
    accepts: paidAccepts(pricing.moneyParse),
    description:
  ...
  "POST /json/repair": {
    accepts: paidAccepts(pricing.jsonRepair),
    description:
      ...
      ...declareDiscoveryExtension({
        inputSchema: {
          ...
          properties: {
            input: {
              type: "string",
              description:
                "The malformed JSON, as a string. May include markdown code fences or surrounding prose. Max 100KB.",
            },
            schema: {
              type: "object",
              description:
                "Optional. Validate the repaired JSON against this schema. Accepts a flat field→type map ({\"n\":\"number\"}) or a JSON-Schema-shaped object ({\"type\":\"object\",\"properties\":{...},\"required\":[...]}). Types: string, number, integer, boolean, object, array, null.",
            },
            allow_llm: {
              type: "boolean",
              description:
                "Default true. When false, deterministic-only: if the tokenizer cannot fix the input, repair_succeeded is false (still a 200).",
            },
            coerce: {
              type: "boolean",
              description:
                "Default false. With a schema supplied, coerce values to the declared types where lossless (\"5\" → 5, \"yes\" → true).",
            },
          },
          required: ["input"],
        },
        ...
        output: {
          schema: {
            type: "object",
            properties: {
              was_valid: { type: "boolean" },
              repair_succeeded: { type: "boolean" },
              repaired: { type: ["object", "array", "null"] },
              method: { type: "string" },
              changes: { type: "array" },
          ...
          example: {
            ...
            repaired: { name: "acme", ports: [80, 443] },
            method: "deterministic",
            changes: [
  ...
  "POST /schema/validate": {
    accepts: paidAccepts(pricing.schemaValidate),
    description:
        ...
        inputSchema: {
          ...
          properties: {
            data: {
              ...
              description:
                "The data to validate: an object, an array, or a JSON string (malformed strings are repaired first). Max 64KB.",
            },
            schema: {
              type: "object",
              description:
                "JSON Schema ({\"type\":\"object\",\"properties\":{...},\"required\":[...]}) or a simplified field→type map ({\"age\":\"number\",\"email\":\"email\",\"tags\":\"string[]\",\"bio\":\"string?\"}). In the map form a field is required unless suffixed with '?'. Formats: email, url, date, date-time, uuid, ipv4, ipv6, hostname.",
            },
            coerce: {
              type: "boolean",
            ...
            repair: {
              type: "boolean",
              description:
                "Default false. When invalid, also return a repaired copy of the data plus is_repaired_valid. Deterministic first (coercion, declared defaults, enum casing, strict extra-key removal).",
            },
            allow_llm: {
              type: "boolean",
        ...
        output: {
          schema: {
            ...
            properties: {
              ...
              repaired: { type: ["object", "array", "null"] },
              is_repaired_valid: { type: ["boolean", "null"] },
          ...
          example: {
            ...
            grade: "A",
            findings: [
              { rule: "data_invalid", deduction: 0, detail: "Data does not satisfy the schema (2 errors)" },
            ],
          },
        },
      }),
  ...
  "POST /schema/map": {
    accepts: paidAccepts(pricing.schemaMap),
    description:
        ...
        inputSchema: {
          ...
          properties: {
            ...
            target_schema: {
              type: "object",
              description:
                "Desired output shape. JSON Schema ({\"type\":\"object\",\"properties\":{...},\"required\":[...]}) or a field→type map ({\"full_name\":\"string\",\"price\":\"number\",\"bio\":\"string?\"}). In the map form a field is required unless suffixed with '?'.",
            },
            hints: {
              type: "object",
              description:
                "Optional explicit mapping, keyed by target field: {\"full_name\":\"name\"} or {\"full_name\":[\"name\",\"contact\"]}. Hints outrank every automatic match.",
            },
            transforms: {
              type: "object",
              description:
                "Optional per-target-field transform: {\"price\":\"toNumber\"}. One of: toNumber, toInteger, toString, toBoolean, toISO, toDate, trim, toLowerCase, toUpperCase, toArray.",
            },
            defaults: {
              type: "object",
            ...
            mode: {
              type: "string",
              description:
                "\"permissive\" (default): an unmapped target field is null + a warning. \"strict\": it is omitted from `mapped`. Either way an incomplete mapping is a 200 with missing_target listed, never an error.",
            },
            allow_llm: {
              type: "boolean",
              description:
                "Default true. Gates semantic mapping. When false, fields the rules cannot match land in missing_target and no model is called.",
            },
          },
          required: ["source", "target_schema"],
        },
        input: {
          source: { name: "Jane Doe", cost: "19.99", internal_id: 7741 },
          target_schema: { full_name: "string", price: "number" },
        },
        bodyType: "json",
        output: {
          schema: {
            ...
            properties: {
              mapped: { type: "object" },
              mapped_fields: { type: "array" },
              unmapped_source: { type: "array" },
              missing_target: { type: "array" },
              type_conversions: { type: "array" },
          ...
          example: {
            mapped: { full_name: "Jane Doe", price: 19.99 },
            mapped_fields: [
              { target: "full_name", source: "name", method: "alias", confidence: 1 },
              { target: "price", source: "cost", method: "alias", confidence: 1 },
            ],
            unmapped_source: ["internal_id"],
            missing_target: [],
            type_conversions: [{ field: "price", from: "string", to: "number" }],
            schema_valid: true,
            warnings: [],
            score: 100,
            grade: "A",
  ...
  "POST /calendar/ics": {
    accepts: paidAccepts(pricing.calendarIcs),
    description:
  ...
  "POST /event-classify": {
    accepts: paidAccepts(pricing.eventClassify),
    description:
  ...
  "POST /event-extract": {
    accepts: paidAccepts(pricing.eventExtract),
    description:
  ...
  "GET /tech-fingerprint/analyze": {
    accepts: paidAccepts(pricing.techFingerprint),
    description:
  ...
  "GET /breach-check/password": {
    accepts: paidAccepts(pricing.breachCheck),
    description:
  ...
  "GET /domain-availability/check": {
    accepts: paidAccepts(pricing.domainAvailability),
    description:
  ...
  "GET /email-intel/analyze": {
    accepts: paidAccepts(pricing.emailIntel),
    description:
      ...
      ...declareDiscoveryExtension({
        ...
        output: {
          ...
          example: {
            ...
            score: 90,
            grade: "A",
            findings: [{ rule: "is_free_provider", deduction: -10, detail: "Domain is a free consumer email provider" }],
          },
        },
      }),
    },
  },
  "GET /email/verify": emailVerifyRouteConfig,
  // POST twin — same handler/price, body-style discovery declare (see the
  // config definitions above the routes object).
  "POST /email/verify": emailVerifyPostRouteConfig,
  "GET /og-scraper/extract": {
    accepts: paidAccepts(pricing.ogScraper),
    description:
  ...
  "GET /page-extract/read": {
    accepts: paidAccepts(pricing.pageExtract),
    description:
  ...
  "GET /web/extract": {
    accepts: paidAccepts(pricing.webExtract),
    description:
  ...
  "GET /phone-intel/analyze": {
    accepts: paidAccepts(pricing.phoneIntel),
    description:
    ...
    extensions: {
      ...declareDiscoveryExtension({
        ...
        output: {
          ...
          // Real response, re-captured 2026-07-19: the example number is the
          // canonical FICTIONAL one (docs must never use a real person's
          // number), and the validator now catches it — every format variant
          // still resolves, the verdict is honestly invalid.
          example: {
            ...
            grade: "F",
            findings: [
              { rule: "unassigned_area_code", deduction: -100, detail: "555 is not an assigned NANP area code — commonly used for fictional numbers" },
            ],
          },
        },
      }),
  ...
  "GET /robots-txt/analyze": {
    accepts: paidAccepts(pricing.robotsTxt),
    description:
  ...
  "GET /rss-parser/fetch": {
    accepts: paidAccepts(pricing.rssParser),
    description:
  ...
  "GET /username-check/lookup": {
    accepts: paidAccepts(pricing.usernameCheck),
    description:
  ...
  "GET /wayback/lookup": {
    accepts: paidAccepts(pricing.wayback),
    description:
  ...
  "GET /domain-age/check": {
    accepts: paidAccepts(pricing.domainAge),
    description:
  ...
  "GET /web/fetch": webFetchRouteConfig,
  // POST twin of /web/fetch — same price/handler, body-style discovery declare
  // (see the config definitions above the routes object).
  "POST /web/fetch": webFetchPostRouteConfig,
  "GET /exa/search": exaSearchRouteConfig,
  "POST /exa/search": exaSearchPostRouteConfig,
  // Brand alias of /exa/search — same handler/price, own description so agents
  // searching the Bazaar for "web search" also retrieve us.
  "GET /web/search": webSearchRouteConfig,
  "POST /web/search": webSearchPostRouteConfig,
  "GET /exa/contents": exaContentsRouteConfig,
  "POST /exa/contents": exaContentsPostRouteConfig,
  "GET /exa/answer": exaAnswerRouteConfig,
  "POST /exa/answer": exaAnswerPostRouteConfig,
  ...
  "POST /github-intel/analyze": githubIntelPostRouteConfig,
  "GET /holidays/check": {
    accepts: paidAccepts(pricing.holidays),
    description:
  ...
  "GET /ip-geo/locate": {
    accepts: paidAccepts(pricing.ipGeo),
    description:
  ...
  "GET /jwt-inspector/decode": {
    accepts: paidAccepts(pricing.jwtInspector),
    description:
  ...
  "POST /lang-detect/analyze": {
    accepts: paidAccepts(pricing.langDetect),
    description:
  ...
  "GET /npm-intel/analyze": {
    accepts: paidAccepts(pricing.npmIntel),
    description:
  ...
  "GET /sitemap-parser/fetch": {
    accepts: paidAccepts(pricing.sitemapParser),
    description:
  ...
  "GET /url-safety/check": {
    accepts: paidAccepts(pricing.urlSafety),
    description:
  ...
  "POST /bulk-domain/check": {
    accepts: paidAccepts(pricing.bulkDomain),
    description:
  ...
  "GET /domain-appraise/estimate": {
    accepts: paidAccepts(pricing.domainAppraise),
    description:
  ...
  "GET /domain-report/analyze": {
    accepts: paidAccepts(pricing.domainReport),
    description:
  ...
  "GET /ip-risk/score": {
    accepts: paidAccepts(pricing.ipRisk),
    description:
      ...
      ...declareDiscoveryExtension({
        ...
        output: {
          schema: {
            ...
            properties: {
              ip: { type: "string" },
              trust_score: { type: "number", description: "0-100, higher = cleaner/safer" },
              risk_score: { type: "number", description: "deprecated alias of trust_score" },
              grade: { type: "string" },
          ...
          // findings couldn't produce its own trust_score — keep this one real).
          example: {
            ip: "8.8.8.8",
            trust_score: 80,
            risk_score: 80,
  ...
  "GET /name-gen/suggest": {
    accepts: paidAccepts(pricing.nameGen),
    description:
  ...
  "GET /tld-price/compare": {
    accepts: paidAccepts(pricing.tldPrice),
    description:
  ...
  "GET /typosquat/scan": {
    accepts: paidAccepts(pricing.typosquat),
    description:
  ...
  "GET /domain-due-diligence": {
    accepts: paidAccepts(pricing.domainDueDiligence),
    description:
  ...
  "GET /domain-report/full": {
    accepts: paidAccepts(pricing.domainReportFull),
    description:
  ...
  "POST /domain/vendor-risk": {
    accepts: paidAccepts(pricing.domainVendorRisk),
    description:
  ...
  "POST /domain/vet": {
    accepts: paidAccepts(pricing.domainVet),
    description:
      // Trimmed from the spec's 456-char copy: the paid-route description cap is
      // 450 (tests/route-description-length.test.ts) because CDP echoes it into
      // the paymentPayload and silently fails settlement past ~505. The funnel
      // parenthetical went; the search terms stay \u2014 they drive Bazaar discovery.
      "Vet and pick a domain in one call \u2014 give candidate names, get back a ranked verdict on which to use and whether it's safe to build on: availability across TLDs, name quality, typosquat exposure, and a factual brand-collision check against similar existing names. Collapses the pick-a-brand-name funnel into one call.",
    mimeType: "application/json",
    extensions: {
      ...declareDiscoveryExtension({
        inputSchema: {
          ...
          properties: {
            ...
            deep_check_top_n: { type: "number" },
          },
          required: ["candidates"],
        },
        input: { candidates: ["satsdesk", "payloop"], tlds: [".com", ".io"] },
        bodyType: "json",
        // NOTE: keep this discovery blob terse \u2014 an oversized example is echoed
        // into the paymentPayload and CDP /verify rejects it (see memory
        // x402-paymentpayload-size-limit). One candidate in the example, schema
        // left description-free.
        output: {
          schema: {
            type: "object",
            properties: {
            ...
            candidates: [
              {
                ...
                availability: { all_tlds_available: true, taken: [], brand_protected_registrar: false, score: 100, source: "bulk-domain" },
                appraisal: { score: 87, tier: "strong", detail: "8 chars, pronounceable, no hyphens/numbers", source: "domain-appraise" },
                typosquat: { registered_lookalikes: 3, available_defensive: 27, risk: "low", source: "typosquat" },
                brand_collision: {
                  available: true,
  ...
  "GET /email-report/full": {
    accepts: paidAccepts(pricing.emailReportFull),
    description:
  ...
  "GET /ip-report/full": {
    accepts: paidAccepts(pricing.ipReportFull),
    description:
        ...
        output: {
          ...
          example: {
            ...
            sections: {
              ...
              risk: { available: true, trust_score: 80, risk_score: 80, recommendation: "allow" },
            },
            sections_ok: 5,
            sections_failed: 0,
  ...
  "GET /url-safety/full": {
    accepts: paidAccepts(pricing.urlSafetyFull),
    description:
...
// Bazaar catalog metadata (serviceName/tags/iconUrl) on every route — the
// structured discovery channel (descriptions' "Search terms:" tails don't
// retrieve). Applied unconditionally so dev boots and tests see it too.
// See src/service-metadata.ts + memory agent-discovery-2026-07.
// Credential-bearing endpoints accept POST as well as GET, so the secret can
// travel in the request BODY instead of the URL.
//
// A password or JWT in a query string is written into edge logs, proxy
// telemetry and shell history before the handler ever runs. /breach-check's
// k-anonymity hashing protects the password from HaveIBeenPwned; it does
// nothing about the transport. Flagged on solana-foundation/pay-skills#269
// ("Passwords Exposed in URLs"); /jwt-inspector/decode is the same class and
// their review missed it.
//
// The GET form is KEPT — all three are live and earning (14, 4 and 10 paid
// calls over 90 days) and removing a verb would break paying callers. POST is
// the documented shape; GET is marked deprecated in the OpenAPI.
const CREDENTIAL_IN_BODY_PATHS = ["/breach-check/password", "/email-report/full", "/jwt-inspector/decode"];

for (const path of CREDENTIAL_IN_BODY_PATHS) {
  const get = (routes as Record<string, any>)[`GET ${path}`];
  if (!get) continue;
  const bazaar = get.extensions?.bazaar;
  if (!bazaar) {
    (routes as Record<string, any>)[`POST ${path}`] = { ...get };
    continue;
  }
  // Move the input from queryParams to body. The key must be DELETED, not set
  // to undefined: the Bazaar indexer strict-validates this blob and
  // `queryParams: undefined` fails as "must be object,boolean", which would
  // silently drop the route from the catalog while it still settled (memory
  // bazaar-strict-schema-validation). Caught by the test suite, not in prod.
  const info = bazaar.info ? { ...bazaar.info } : undefined;
  if (info?.input) {
    const { queryParams, ...restInput } = info.input as Record<string, unknown>;
    info.input = { ...restInput, method: "POST", body: queryParams ?? (info.input as any).body };
  }
  const schema = bazaar.schema ? { ...bazaar.schema } : undefined;
  const inputSchema = schema?.properties?.input;
  if (inputSchema?.properties) {
    const { queryParams: qpSchema, ...restProps } = inputSchema.properties as Record<string, unknown>;
    schema!.properties = {
      ...schema!.properties,
      input: {
        ...inputSchema,
        properties: { ...restProps, method: { type: "string", enum: ["POST"] }, body: qpSchema ?? (inputSchema.properties as any).body },
      },
    };
  }
  (routes as Record<string, any>)[`POST ${path}`] = {
    ...get,
    extensions: { ...get.extensions, bazaar: { ...bazaar, ...(info ? { info } : {}), ...(schema ? { schema } : {}) } },
  };
}

applyServiceMetadata(routes);

export { routes };

/** Every route router, in NetIntel's declaration order. */
export const routers = [dnsRouter, sslRouter, sslCertQuickRouter, subnetRouter, redirectRouter, securityHeadersRouter, emailAuthRouter, cloudFingerprintRouter, schemaParseRouter, classifyRouter, messagesRouter, aiImageAssetsRouter, openaiGpt4oRouter, openaiGpt41Router, openaiGpt41MiniRouter, openaiGpt4oMiniRouter, openaiGpt41NanoRouter, openaiGpt55Router, openaiGpt54Router, openaiGpt54MiniRouter, openaiGpt54NanoRouter, openaiGpt51Router, openaiGpt5NanoRouter, v1OpenAiCompatRouter, v1EmbeddingsRouter, embeddingsLocalRouter, semanticRankRouter, openaiGpt52Router, openaiGpt56SolRouter, openaiGpt56TerraRouter, openaiGpt56LunaRouter, contentModerateRouter, entityExtractRouter, extractAddressRouter, extractContactRouter, extractInvoiceRouter, extractResumeRouter, extractTableRouter, markdownCleanRouter, normalizeJsonRouter, textToJsonRouter, sentimentRouter, textSummarizeRouter, textChunkRouter, textStatsRouter, translateLongRouter, translateShortRouter, translateStructuredRouter, translateBatchRouter, asnLookupRouter, whoisRdapRouter, certTransparencyRouter, dnsPropagationRouter, dnssecRouter, ipBlacklistRouter, techFingerprintRouter, breachCheckRouter, domainAvailabilityRouter, emailIntelRouter, emailVerifyRouter, ogScraperRouter, pageExtractRouter, webExtractRouter, webFetchRouter, exaSearchRouter, exaContentsRouter, exaAnswerRouter, phoneIntelRouter, robotsTxtRouter, rssParserRouter, usernameCheckRouter, waybackRouter, ipReputationRouter, cronParserRouter, currencyExchangeRouter, cryptoMarketRouter, gasPriceRouter, cryptoPriceRouter, cryptoOhlcRouter, currencyBatchRouter, currencyHistoryRouter, tokenInfoRouter, walletBalanceRouter, walletIntelRouter, ibanValidateRouter, marketSnapshotRouter, convertRouter, moneyParseRouter, jsonRepairRouter, schemaValidateRouter, schemaMapRouter, calendarIcsRouter, eventClassifyRouter, eventExtractRouter, githubIntelRouter, holidaysRouter, ipGeoRouter, weatherCurrentRouter, weatherForecastRouter, predictionMarketsRouter, predictionMarketRouter, jwtInspectorRouter, langDetectRouter, npmIntelRouter, sitemapParserRouter, urlSafetyRouter, bulkDomainRouter, domainAgeRouter, domainAppraiseRouter, domainReportRouter, ipRiskRouter, nameGenRouter, tldPriceRouter, typosquatRouter, domainDueDiligenceRouter, domainReportFullRouter, emailReportFullRouter, ipReportFullRouter, urlSafetyFullRouter, domainVendorRiskRouter, domainVetRouter];
